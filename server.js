...
app.post("/api/forgot-password", async (req, res) => {
  try {
    ...
    user.reset_token_expires = moment()
      .add(1, "hour")
      .tz("Africa/Nairobi")
      .format();
    await user.save();
    ...
    await sendEmail(
      user.email,
      "Password Reset",
      `Your password reset link is: ${process.env.FRONTEND_URL}/reset-password?token=${resetToken}`,
    );
...
app.get("/api/verify-token", (req, res) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => {
    if (err) {
      return res
        .status(401)
        .json({ valid: false, error: "Failed to authenticate token" });
    }
    res.json({ valid: true });
  });
});
  ...
  try {
    ...
    const result = await sequelize.transaction(async (t) => {
      ...
      const newOutstandingBalance =
        parseFloat(customer.outstandingLoan) - parseFloat(amount);
      ...
      const [updatedRows] = await Customer.update(
        { outstandingLoan: sequelize.literal(`outstandingLoan - ${amount}`) },
        {
          where: {
            id: customerId,
            outstandingLoan: { [Sequelize.Op.gte]: amount },
          },
          transaction: t,
        },
      );
    ...
    res.status(500).json({
      success: false,
      error: "An error occurred while processing the repayment",
    });
  }
});
